...
class AI():
    ...
    def tools(obj, tools, agents):
        ...
        for tool in tools:
            if tool in obj._tools:
                ...
                class_funcs = [attr_name for attr_name in dir(tool) if
                               not attr_name.startswith('_') and callable(getattr(tool, attr_name)) and getattr(tool,
                                                                                                                attr_name).__doc__]
    ...
    def run(self, query, llm, tools=[],  verbose=False):
        ...
        agent = create_tool_calling_agent(
            llm,
            tools=self.tools(self, tools,[]),
            prompt=prompt
        )
        agent_executor = AgentExecutor(agent=agent, tools=self.tools(self, tools,[]))
